	...
	foreach($auto->getProperties() as $prop){
		...
		switch($prop){
			...
			case 'first_circulation_date':
				if($value == '' && in_array($prop, array_keys($required))){
					...
					if (checkdate($array_jours[2],$array_jours[1],$array_jours[3])){
						...
						$error_detected[] = str_replace('%s', $auto->getPropName($prop), _T("- Non valid date for %s!"));
				} else {
					$error_detected[] = str_replace('%s', $auto->getPropName($prop), _T("- Wrong date format for %s (dd/mm/yyyy)!"));
				}
				break;
			//numeric values
			case 'mileage':
				...
				if( $value > 0 )
					...
					switch($prop){
						...
						case 'model':
							...
							break;
						default:
							$log->log('Unable to retrieve the textual value for prop `' . $prop . '`', PEAR_LOG_INFO);
							$name = '(unknow)';
					}
					$error_detected[] = str_replace('%s', '<a href="#' . $prop . '">' . $name . '</a>', _T("- You must choose a %s in the list"));
				}
				break;
			case 'owner':
				$value = get_numeric_form_value($prop, 0);
			...
			default:
				...
				$log->log('Trying to edit an Auto property that is not catched in the source code! (prop is: ' . $prop . ')', PEAR_LOG_ERR);
				break;
		}//switch
	}//foreach
