...
{foreach from=$autos item=auto name=autos_list}
    {assign var='brand' value=$auto->model->obrand}
                <tr>
                    ...
                    <td class="tbl_line_{if $smarty.foreach.autos_list.iteration % 2 eq 0}even{else}odd{/if}"><a href="{if $show_mine eq 1}my_{/if}vehicles_edit.php?id_car={$auto->id}">{$auto->name}</a></td>
                    <td class="tbl_line_{if $smarty.foreach.autos_list.iteration % 2 eq 0}even{else}odd{/if}"><a href="{if $show_mine eq 1}my_{/if}vehicles_edit.php?id_car={$auto->id}">{$brand->value}</a></td>
                    <td class="tbl_line_{if $smarty.foreach.autos_list.iteration % 2 eq 0}even{else}odd{/if}"><a href="{if $show_mine eq 1}my_{/if}vehicles_edit.php?id_car={$auto->id}">{$auto->model->model}</a></td>
                    <td class="center nowrap tbl_line_{if $smarty.foreach.autos_list.iteration % 2 eq 0}even{else}odd{/if}">
